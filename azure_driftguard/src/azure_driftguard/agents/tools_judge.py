...
def judge_patch(
    *,
    ...
    verdict = _call_judge_model(cfg.openai.deployment_judge, prompt)
    if verdict is None:
...
def _call_judge_model(model: str, prompt: str) -> dict[str, Any] | None:
    """Call the judge model via Azure OpenAI. Return parsed JSON or None."""
    from ..common.llm_azure_openai import AzureOpenAIUnavailable, generate_json

    try:
        return generate_json(model, _JUDGE_SYSTEM, prompt, temperature=0.0)
    except AzureOpenAIUnavailable:
        logger.warning("Azure OpenAI unavailable; judge skipped")
        return None
