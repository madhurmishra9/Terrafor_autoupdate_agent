...
Uses Azure OpenAI text embeddings. When the SDK is unavailable, returns relevant=True
...
def _managed_topics() -> list[str]:
    ...
    return topics or ["Terraform provider", "azurerm_ resource"]

_RELEVANCE_THRESHOLD = 0.55


def _embed(texts: list[str]) -> list[list[float]] | None:
    from ..common.llm_azure_openai import embed
