...
aliases: [Azure Blob Storage, Blob Storage]              # keywords to match in release notes
provider: azurerm
...
related_resources:                       # SECOND-LEVEL resources where features hide
  - azurerm_storage_blob        # e.g. cache_control lives here, not the primary
  - azurerm_storage_container
module_paths: [modules/storage]              # edits are confined to these paths
...
feeds:                                   # the product owner's own feed(s), optional
  - url: https://azurecomcdn.azureedge.net/en-us/updates/feed/
    format: rss
relevance_topics: [Blob cache control]
```

Only `name` is required. `related_resources` declares the product's full
resource family so a feature on a secondary resource (e.g. `cache_control` on
`azurerm_storage_blob`) is found and patched on the right resource — see
