...
from azure_driftguard.common.logging_setup import get_logger
...
def _load_fixture(d: Path) -> tuple[dict[str, Any], ExpectedOutcome, str]:
    ...
    expected = ExpectedOutcome(
        ...
        provider=exp_raw.get("provider", "azurerm"),
...
def score_fixture(d: Path, mode: str = "deterministic") -> FixtureResult:
    from azure_driftguard.agents import tools_judge, tools_terraform
    ...
    if product:
        try:
            from azure_driftguard.common.product_registry import registry
...
def _generate_live(note: dict[str, Any], expected: ExpectedOutcome) -> str | None:
    ...
    try:
        from azure_driftguard.orchestration.azure_orchestrator import run_pipeline  # noqa: F401
