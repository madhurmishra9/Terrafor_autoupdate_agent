...
def judge_patch(
    *,
    ...
    verdict = _call_judge_model(cfg.bedrock.model_judge, prompt)
    if verdict is None:
...
def _call_judge_model(model: str, prompt: str) -> dict[str, Any] | None:
    """Call the judge model via Bedrock. Return parsed JSON or None."""
    from ..common.llm_bedrock import BedrockUnavailable, generate_json

    try:
        return generate_json(model, _JUDGE_SYSTEM, prompt, temperature=0.0)
    except BedrockUnavailable:
        logger.warning("Bedrock unavailable; judge skipped")
        return None
