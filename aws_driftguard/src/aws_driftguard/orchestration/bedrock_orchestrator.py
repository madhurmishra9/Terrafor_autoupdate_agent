"""Native Bedrock orchestrator (runnable Converse tool-use path).

Runs the 7-stage pipeline sequentially. Each stage is a Bedrock Converse
...
def _model_for(spec: StageSpec) -> str:
    ...
    return cfg.model_fast if spec.model_tier == "fast" else cfg.model
...
def _run_stage(spec: StageSpec, state: dict[str, Any]) -> None:
    ...
    from ..common.llm_bedrock import BedrockUnavailable, converse_with_tools
    ...
    tools = tool_registry.specs_for(spec.tools)

    try:
        final = converse_with_tools(
        ...
        logger.error("Bedrock unavailable, halting: %s", exc)
        halt_pipeline(state, "bedrock_unavailable")
        state[spec.output_key] = STOP_SENTINEL
        return
...
def run_pipeline(prompt: str) -> dict[str, Any]:
    ...
    logger.info("Starting AWS DriftGuard pipeline (Bedrock converse mode)")
