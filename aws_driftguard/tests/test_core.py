...
from aws_driftguard.common import adf  # noqa: E402
from aws_driftguard.common import taxonomy  # noqa: E402
from aws_driftguard.common import state  # noqa: E402
...
def test_adf_release_description_has_blocks():
    d = adf.release_ticket_description(
        product="Amazon RDS", version="6.0.0", release_date="2026-01-10",
...
def test_stop_guard_stamps_output_key():
    # In the native orchestrator, a halted pipeline stamps the stage output_key
    # with the STOP sentinel and skips the stage. Verify the state contract.
    st: dict = {}
    assert not state.is_halted(st)
    state.halt_pipeline(st, "x")
    assert state.is_halted(st)
    # Simulate the orchestrator's stop-guard stamping.
...
def test_adf_flatten_in_jira_client():
    from aws_driftguard.common.jira_client import _adf_to_text
