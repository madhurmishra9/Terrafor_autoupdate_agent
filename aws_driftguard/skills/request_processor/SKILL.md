...
task is to fetch AWS release notes (What's New), normalise them, and write a structured list
...
1. Call `list_feeds` to discover WHICH feeds to fetch (shared cloud feed
   ...
   `triggered_at` time.
2. For each feed, call `fetch_aws_release_notes(feed_url=<feed.url>)` to retrieve its entries.
...
5. Optionally call `list_aws_products` to filter to products of interest.
