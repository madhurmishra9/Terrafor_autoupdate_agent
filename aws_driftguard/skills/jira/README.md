...
A AWS DriftGuard change must be tracked in Jira before a PR is raised, so
...
|-----------|------|---------|
| Agent definition | `src/aws_driftguard/agents/definitions.py` | LlmAgent constructor for JiraAgent |
| Skill (instructions) | `skills/jira/SKILL.md` | Behaviour, 3-tier order, ADF rules |
| Tools | `src/aws_driftguard/agents/tools_jira.py` | search/create/comment tools |
| Jira client | `src/aws_driftguard/common/jira_client.py` | 3-tier fallback implementation |
| ADF builders | `src/aws_driftguard/common/adf.py` | ADF document construction |
| Connectivity guard | `src/aws_driftguard/agents/guards.py` | `jira_connectivity_guard` |
| Taxonomy | `src/aws_driftguard/common/taxonomy.py` | classification → issue type map |
