...
the Terraform `hashicorp/aws` (or `aws`) provider, and whether org
   ...
   d. **Second-level resolution (do this — it prevents hallucination):** extract
      ...
      attribute.
        - If `resolved=true`, use `owner_resources` as the resource to change —
          ...
          context" feature for Amazon S3 resolves to
          `aws_s3_object`, not `aws_s3_bucket`.
     ...
     "changes": [
       {
         ...
         "argument": "object_lock_configuration",
         "description": "...",
