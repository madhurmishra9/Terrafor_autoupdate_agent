# Implementation Plan — AWS DriftGuard Agent (Skills Architecture)

This plan describes how to deploy, validate, and operate the skills-driven
AWS DriftGuard Agent in a production GKE environment. It assumes the code in this
repository is the starting point.

## 0. Prerequisites

- ECS Fargate or EKS cluster (Autopilot or Standard) with IRSA (EKS) or task role (ECS) enabled.
- A Google Service Account (GSA) bound to the Kubernetes Service Account (KSA)
  with roles: `bedrock:InvokeModel, RDS connect, secretsmanager:GetSecretValue`,
  `roles/secretmanager.secretAccessor`.
- A RDS Postgres instance reachable via the RDS endpoint.
...
python -c "from aws_driftguard.common.jira_client import JiraClient; print(JiraClient().probe())"
```
...
Validate:
```bash
python -c "from aws_driftguard.common.github_client import GitHubClient; print(GitHubClient().probe())"
```
...
```bash
psql "$(python -c 'from aws_driftguard.common.config import get_config; print(get_config().cloudsql.dsn)')" \
  -f deploy/schema.sql
```
...
```bash
# Build
docker build -t REGION-docker.pkg.dev/PROJECT/aws-driftguard/agent:$(git rev-parse --short HEAD) .

# Push
docker push REGION-docker.pkg.dev/PROJECT/aws-driftguard/agent:TAG
