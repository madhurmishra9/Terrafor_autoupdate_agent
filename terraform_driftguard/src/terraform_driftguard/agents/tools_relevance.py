...
Uses Vertex text embeddings. When the SDK is unavailable, returns relevant=True
...
def _managed_topics() -> list[str]:
    ...
    # Always include generic provider-resource cues so the filter still works
    # even before any product manifests are added.
    return topics or ["Terraform provider", "google_ resource"]
...
def _embed(texts: list[str]) -> list[list[float]] | None:
    ...
    cfg = get_config()
    try:
        client = genai.Client(vertexai=True, project=cfg.gcp.project, location=cfg.gcp.location)
        resp = client.models.embed_content(model="text-embedding-005", contents=texts)
