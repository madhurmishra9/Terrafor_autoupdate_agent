...
the Terraform `hashicorp/google` (or `google-beta`) provider, and whether org
   ...
   d. **Second-level resolution (do this — it prevents hallucination):** extract
      ...
      attribute.
        - If `resolved=true`, use `owner_resources` as the resource to change —
          ...
          context" feature for Cloud Storage resolves to
          `google_storage_bucket_object`, not `google_storage_bucket`.
