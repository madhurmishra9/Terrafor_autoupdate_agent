...
the GenerateAgent so that content generation and repository mutation remain
separate concerns, and from TicketAgent so ticket creation and PR creation can
...
|-----------|------|---------|
| Agent definition | `src/terraform_driftguard/agents/definitions.py` | LlmAgent constructor for PublishAgent |
| Skill | `skills/publish/SKILL.md` | Behaviour, title rules, linkback |
| Tools | `src/terraform_driftguard/agents/tools_pr.py` | branch/push/PR/linkback tools |
| GitHub client | `src/terraform_driftguard/common/github_client.py` | PAT + App auth, api/v3 |
| Artifact store | `src/terraform_driftguard/agents/artifacts.py` | loads patched files |
| Connectivity guard | `src/terraform_driftguard/agents/guards.py` | `github_connectivity_guard` |
...
- `tests/test_taxonomy.py` — prefix mapping correctness (shared with TicketAgent).
- `tests/test_pr_title.py` — title format from classification.

## Cross-references

- Upstream: [TicketAgent](../jira/README.md)
