...
A Terraform DriftGuard change must be tracked in Jira before a PR is raised, so
...
classification → issue-type → PR-prefix mapping that PRAgent consumes. It is a
...
- Position: Step 6 of 7, after TerraformAgent, before PRAgent.
...
|-----------|------|---------|
| Agent definition | `src/terraform_driftguard/agents/definitions.py` | LlmAgent constructor for JiraAgent |
| Skill (instructions) | `skills/jira/SKILL.md` | Behaviour, 3-tier order, ADF rules |
| Tools | `src/terraform_driftguard/agents/tools_jira.py` | search/create/comment tools |
| Jira client | `src/terraform_driftguard/common/jira_client.py` | 3-tier fallback implementation |
| ADF builders | `src/terraform_driftguard/common/adf.py` | ADF document construction |
| Connectivity guard | `src/terraform_driftguard/agents/guards.py` | `jira_connectivity_guard` |
| Taxonomy | `src/terraform_driftguard/common/taxonomy.py` | classification → issue type map |
...
- Upstream: [TerraformAgent](../terraform/README.md)
- Downstream: [PRAgent](../pr/README.md)
