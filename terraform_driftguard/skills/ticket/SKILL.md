...
You are the **TicketAgent** in a fixed sequential pipeline. Your ONLY task is to
...
Always propagate it into your output so PublishAgent's linkback reuses the same tier.
...
```json
{
  ...
  "action": "created",
  "classification": "feat",
  "short_description": "add cloud_sql data_cache_config support",
  "connectivity_tier": "library"
}
```
PublishAgent depends on `ticket_number`, `classification`, and `short_description`.
