...
`TerraformDriftGuardPipelineAgent` is an ADK `SequentialAgent` running on Vertex AI
(Gemini 2.5), deployed on GKE with sidecars. Seven sub-agents run in fixed order.
Each loads its instruction from a versioned `SKILL.md`.

```
TerraformDriftGuardPipelineAgent (SequentialAgent)
...
└── sub_agents:
      1 RequestProcessorAgent  → release_notes
      2 ClassificationAgent    → classification_result
      3 ChangeAnalyserAgent    → change_analyser_result
      4 DecisionMakerAgent     → decision_maker_result
      5 TerraformAgent         → terraform_result
      6 JiraAgent              → jira_result
      7 PRAgent                → pr_result
```
...
|-----|-----------|-----------|
| `release_notes` | RequestProcessorAgent | cb_before_pipeline |
| `classification_result` | ClassificationAgent | cb_before_pipeline |
| `change_analyser_result` | ChangeAnalyserAgent | cb_before_pipeline |
| `decision_maker_result` | DecisionMakerAgent | cb_before_pipeline |
| `terraform_result` | TerraformAgent | cb_before_pipeline |
| `jira_result` | JiraAgent | cb_before_pipeline |
| `pr_result` | PRAgent | cb_before_pipeline |
| `pipeline_mode` | RequestProcessorAgent (fetch-only) | cb_before_pipeline |
...
and the agent does nothing. DecisionMaker, Jira, and PR additionally chain a
connectivity guard:

- DecisionMaker, PR → `github_connectivity_guard`
...
PRAgent's linkback comment reuses the same mechanism. All rich text is ADF
...
```
Pod
├── terraform-driftguard (main)      Vertex AI client, ADK runtime, skills mount
├── cloud-sql-proxy (sidecar)      127.0.0.1:5432 → Cloud SQL
└── mcp-server (sidecar)           localhost:8080 → GitHub tools
```

Workload Identity binds the KSA to a GSA holding `roles/cloudsql.client` and
...
|-----------|------------|--------|
| Feed fetch fails | RequestProcessor `[STOP]` | halt |
| CloudSQL down | Classification tool error | halt |
...
| No actionable changes | ChangeAnalyser `changes_required:false` | clean no-op downstream |
