# Implementation Plan — Terraform DriftGuard Agent (Skills Architecture)

This plan describes how to deploy, validate, and operate the skills-driven
Terraform DriftGuard Agent in a production GKE environment. It assumes the code in this
...
- A Google Service Account (GSA) bound to the Kubernetes Service Account (KSA)
  with roles: `roles/cloudsql.client`, `roles/aiplatform.user`,
  `roles/secretmanager.secretAccessor`.
- A CloudSQL Postgres instance reachable via the Cloud SQL Auth Proxy.
...
python -c "from terraform_driftguard.common.jira_client import JiraClient; print(JiraClient().probe())"
```
...
Validate:
```bash
python -c "from terraform_driftguard.common.github_client import GitHubClient; print(GitHubClient().probe())"
```
...
```bash
psql "$(python -c 'from terraform_driftguard.common.config import get_config; print(get_config().cloudsql.dsn)')" \
  -f deploy/schema.sql
```
...
```bash
# Build
docker build -t REGION-docker.pkg.dev/PROJECT/terraform-driftguard/agent:$(git rev-parse --short HEAD) .

# Push
docker push REGION-docker.pkg.dev/PROJECT/terraform-driftguard/agent:TAG
